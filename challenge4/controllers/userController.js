...
const addCustomer = async (req, res) => {
  ...
  try {
    ...
    return response.res200(res, customer);
  } catch (err) {
    if (err.code === 'P2002' && err.meta.target.includes('username')) {
      return res.status(400).json({ error: 'Someone is already using the username you have chosen. Please try using another one instead.' });
    }
